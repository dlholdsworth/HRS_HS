...
def run_wavelength_cal(calflux, rough_wls=None, our_wavelength_solution_for_order=None,
...
#            os.makedirs(self.save_diagnostics_dir + '/order_diagnostics')
    ...
    if quicklook == False:
        order_list = np.arange(calflux.shape[0])
        ...
        poly_soln, wls_and_pixels, orderlet_dict = fit_many_orders(
            ...
            our_wavelength_solution_for_order=our_wavelength_solution_for_order, print_update=True, plt_path=save_diagnostics_dir
        )
        rough_wls=fits.getdata("/Users/daniel/Desktop/SALT_HRS_DATA/Blu/2023/0728/reduced/HRS_E_W_bogH202307280013.fits")
        rough_wls = rough_wls[0]
        # make a plot of all of the precise new wls minus the rough input  wls
        if save_diagnostics_dir is not None and rough_wls is not None:
            ...
            if cal_type != 'Etalon':
                ...
                plt.tight_layout()
                plt.savefig(
                    '{}/all_wls.png'.format(save_diagnostics_dir),
                    dpi=350
                )
...
def air_to_vac(lines):

    sigma2 = (1e4 / lines) ** 2  # Compute wavenumbers squared
    fact = (
        1e0
        + 8.336624212083e-5
    ...
    return lines * fact  # Convert to vac wavelength and send back

def fit_many_orders(cal_flux, order_list, rough_wls=None, comb_lines_angstrom=None,
    ...
    fit_iterations = 5
    sigma_clip = 2.1
    if cal_flux.shape[0] == 42:
        rough_wls=fits.getdata("/Users/daniel/Desktop/SALT_HRS_DATA/Blu/2023/0728/reduced/HRS_E_W_bogH202307280013.fits")
        linelist = np.load("/Users/daniel/Documents/Work/SALT_Pipeline/PyReduce-HRS/datasets/HRS/reduced/hrs_hs.H.linelist.npz",allow_pickle=True)
        linelist = linelist['cs_lines']
        good_ord,good_pix,good_wav = np.loadtxt("Good_thar_pix_locations.txt",usecols=(0,1,2),unpack=True)
    ...
    for order_num in order_list:
        ...
        rough_wls_order = rough_wls[0][order_num]
        ll=np.where(linelist['order'] == order_num)[0]
        expected_peak_locs = linelist['posm'][ll]
        expected_peak_wave = linelist['wlc'][ll]
        
        ll=np.where(good_ord == order_num)[0]
        expected_peak_locs = good_pix[ll]
        expected_peak_wave = good_wav[ll]
        
        #Convert to vacuum
        #expected_peak_wave = air_to_vac(expected_peak_wave)
        
#        if order_num == 13:# or order_num == 25:
#            expected_peak_wave = expected_peak_wave[:-1]
#            plt.plot(order_flux)
#            plt.vlines(expected_peak_locs,0,10000,'r')
#            plt.show()
        ...
        orderlet_dict[order_num]['echelle_order'] = \
            dlh_utils.get_hrs_echelle_order(np.median(rough_wls_order))
        ...
        line_wavelengths = expected_peak_wave
        ...
        wls, gauss_coeffs, lines_dict = line_match(
        ...
        if cal_type != 'Etalon':
            ...
            polynomial_wls, leg_out = fit_polynomial(
                ...
                plot_path=order_plt_path, fit_iterations=fit_iterations,
            ...
            rel_precision, abs_precision = calculate_rv_precision(
    ...
    if cal_type != 'Etalon':
        ...
        print('\n\n\nOverall absolute precision (all orders): {:2.2f} m/s\n\n\n'.format(overall_std_error/100))
...
def line_match(flux, linelist, line_pixels_expected, plot_toggle, savefig, gaussian_fit_width=10):
    """
    ...
    for i in np.arange(num_input_lines):
        ...
        if peak_pixel < len(flux) and flux[peak_pixel] <= 1e6:
            ...
            result, line_dict = fit_gaussian_integral(
        ...
        for i, ax in enumerate(ax_list):
            ...
            for j in np.arange(num_input_lines-missed_lines):
                ...
                if (
                    ...
                    gaussian_fit = integrate_gaussian(
...
def fit_gaussian_integral(x, y):
    ...
    chi_2_threshold =2
    chi_squared_threshold = int(chi_2_threshold)
...
def fit_polynomial(wls, rough_wls_order, peak_wavelengths_ang, order_list, n_pixels, fitted_peak_pixels, fit_iterations=5, sigma_clip=2.1, peak_heights=None, plot_path=None):
    """
    ...
    fit_type = 'legendre'
    cal_type = 'ThAr'
    if fit_type.lower() not in ['legendre', 'spline']:
        raise NotImplementedError("Fit type must be either legendre or spline")
    
    if fit_type.lower() == 'legendre' or fit_type.lower() == 'spline':
        ...
        if plot_path is not None and cal_type =='ThAr':
            approx_dispersion = (our_wavelength_solution_for_order[1600] - our_wavelength_solution_for_order[1700])/100
            fig, ax1 = plt.subplots(tight_layout=True, figsize=(8, 4))
            
            # Range of interest b/c CCF chops off first/last 500 pixels
            pixel_range = np.arange(100, 2048-100)
            rough_wls_int_range = rough_wls_int(pixel_range)
            wavelength_solution_range = our_wavelength_solution_for_order[100:2048-100]
...
def calculate_rv_precision(
    fitted_peak_pixels, wls, leg_out, rough_wls, our_wavelength_solution_for_order, rough_wls_order,
    ...
    if print_update:
        print('Absolute standard error (this order): {:.2f} m/s'.format(abs_precision_cm_s/100))
        print('Relative standard error (this order): {:.2f} m/s'.format(rel_precision_cm_s/100))
...
file="/Users/daniel/Desktop/SALT_HRS_DATA/Blu/2023/0728/reduced/HRS_E_W_bogH202307280013.fits"

hdr=fits.open(file)
header=hdr[0].header
HRS_data_all=hdr[0].data
HRS_data=HRS_data_all[1]
nord=HRS_data.shape[0]
hdr.close
        
wl_soln, wls_and_pixels, orderlet_dict = run_wavelength_cal(HRS_data, rough_wls=None, our_wavelength_solution_for_order=None, peak_wavelengths_ang=None, lfc_allowed_wls=None,input_filename=None)

plt.show()
for ord in range(nord):
    plt.plot(HRS_data_all[0][ord],HRS_data_all[1][ord],ls='--')
    plt.plot(wl_soln[ord,:],HRS_data_all[1][ord])
    HRS_data_all[0][ord] = wl_soln[ord,:]
    
plt.show()

#Create new FITS file for the data.
hdu = fits.PrimaryHDU(data=HRS_data_all,header=header)
file_out=str("./test.fits")
